// vsop.js — VSOP87D 축약 급수(Meeus Appendix III)로 태양 겉보기 황경 계산
// 정확도: 각초(arcsecond) 수준 → 절기 시각 오차 수 초 이내
...
/**
 * 태양의 겉보기 황경 (도). jde 는 TT 기준 율리우스일.
 */
export function apparentSunLongitude(jde) {
  const tau = (jde - 2451545.0) / 365250.0;
  const t2 = tau * tau, t3 = t2 * tau, t4 = t3 * tau, t5 = t4 * tau;

  const L = (series(L0, tau) + series(L1, tau) * tau + series(L2, tau) * t2
    + series(L3, tau) * t3 + series(L4, tau) * t4 + series(L5, tau) * t5) / 1e8;
  const R = (series(R0, tau) + series(R1, tau) * tau + series(R2, tau) * t2
    + series(R3, tau) * t3 + series(R4, tau) * t4) / 1e8;

  // 지구 황경 → 태양 황경
  let theta = mod360(L * DEG + 180);

  // FK5 보정
  const T = (jde - 2451545.0) / 36525.0;
  theta -= 0.09033 / 3600;

  // 장동(nutation in longitude)
  ...
  const dPsi = (-17.20 * Math.sin(Om * rad) - 1.32 * Math.sin(2 * Ls * rad)
    - 0.23 * Math.sin(2 * Lm * rad) + 0.21 * Math.sin(2 * Om * rad)) / 3600;
